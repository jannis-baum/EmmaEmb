...
from emma.knn_analysis import (get_knn_alignment_scores, 
...
def paired_wilcoxon_test_penguin(all_results_df, embedding_1, embedding_2):
    ...
    embedding_1_data = all_results_df[all_results_df['Embedding'] == embedding_1]['Fraction']
    embedding_2_data = all_results_df[all_results_df['Embedding'] == embedding_2]['Fraction']

    # Check if both embeddings have the same length (for paired test)
    if len(embedding_1_data) != len(embedding_2_data):
        raise ValueError("The two embeddings must have the same number of data points for a paired test.")

    # Perform the paired Wilcoxon test
    results = pg.wilcoxon(embedding_1_data, embedding_2_data, alternative='greater')
...
if ("Fig_A" in figures_to_be_plotted) or ('Fig_B' in figures_to_be_plotted):
    ...
    knn_alignment_scores = get_knn_alignment_scores(
        ...
        k_neighbors=100, distance_metric=distance_metric, 
    )
    
    # Perform Wilcoxon tests for each pair and print the results
    w_stat, p_value, rbc = paired_wilcoxon_test_penguin(knn_alignment_scores, "ESM2", "ProstT5")
    print(f"ESM2 vs ProstT5 - Test Statistic: {w_stat}, P-Value: {p_value}, Rank-biserial correlation: {rbc}")

    w_stat, p_value, rbc = paired_wilcoxon_test_penguin(knn_alignment_scores, "Ankh", "ESM2")
    print(f"Ankh vs ESM2 - Test Statistic: {w_stat}, P-Value: {p_value}, Rank-biserial correlation: {rbc}")

    w_stat, p_value, rbc = paired_wilcoxon_test_penguin(knn_alignment_scores, "ProtT5", "Ankh")
    print(f"ProtT5 vs Ankh - Test Statistic: {w_stat}, P-Value: {p_value}, Rank-biserial correlation: {rbc}")
